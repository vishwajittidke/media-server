...
# FIX: Allow Cloudinary SDK to handle CLOUDINARY_URL initialization natively
if settings.CLOUDINARY_URL:
...
def list_files(
    ...
    for f in files:
        ...
        if f.storage_path and f.storage_path.startswith("http"):
            ...
            # FIX: Ensure explicit paths are exposed to prevent absolute URL resolution bugs on the client UI
            f_dict["thumbnail_url"] = f"/api/v1/files/static/thumbnails/{f.stored_name}"
            f_dict["preview_url"] = f"/api/v1/files/static/previews/{f.stored_name}"
        result.append(f_dict)
